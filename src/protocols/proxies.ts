...
import { dataToUint8Array, joinBytes, concatBytes } from '../utils/bytes.js';
import { safeClose, safeRelease } from '../utils/helpers.js';
import type { DoHAnswer, LogFn, ParsedProxyAddress } from '../types.js';

const textEncoder = new TextEncoder();
const textDecoder = new TextDecoder();

export function createProxyConnectors({
	getParsedProxy,
	validDataLength,
	log = () => {},
	dohQuery
	...
	validDataLength: (value: unknown) => number;
	log?: LogFn;
	dohQuery: (domain: string, recordType: string) => Promise<DoHAnswer[]>;
}) {
	async function socks5Connect(targetHost, targetPort, initialData) {
		const { username, password, hostname, port } = getParsedProxy();
	...
	async function httpConnect(targetHost, targetPort, initialData, HTTPSproxy = false) {
		...
		try {
			...
			while (headerEndIndex === -1 && bytesRead < 8192) {
				...
				if (done || !value) throw new Error(`${HTTPSproxy ? 'HTTPS' : 'HTTP'} proxyReturning CONNECT closeConnBeforeResponse`);
			...
			if (headerEndIndex === -1) throw new Error('proxy CONNECT headerTooLongOrInvalid');
	...
	async function httpsConnect(targetHost, targetPort, initialData) {
		...
		try {
			try {
				...
				log(`[HTTPSproxy] AES-GCM TLS handshake failed, fallback ChaCha20 compatMode: ${error?.message || error}`);
			...
			while (headerEndIndex === -1 && bytesRead < 8192) {
				...
				if (!value) throw new Error('HTTPS proxyReturning CONNECT closeConnBeforeResponse');
			...
			if (headerEndIndex === -1) throw new Error('HTTPS proxy CONNECT headerTooLongOrInvalid');
			const statusMatch = decoder.decode(responseBuffer.slice(0, headerEndIndex)).split('\r\n')[0].match(/HTTP\/\d\.\d\s+(\d+)/);
			const statusCode = statusMatch ? parseInt(statusMatch[1], 10) : NaN;
			if (!Number.isFinite(statusCode) || statusCode < 200 || statusCode >= 300) throw new Error(`Connection failed: HTTP ${statusCode}`);
			...
			const settleClosed = (settle, value = undefined) => {
				if (!closedSettled) {
					closedSettled = true;
					settle(value);
				}
	...
	return { socks5Connect, httpConnect, httpsConnect, turnConnect, sstpConnect, isIPHostname };
}
