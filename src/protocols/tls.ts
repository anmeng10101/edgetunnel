import { dataToUint8Array, concatBytes } from '../utils/bytes.js';
...
const CIPHER_SUITES_BY_ID = new Map<number, TlsCipherConfig>([
...
async function generateKeyShare(group = "P-256") {
	...
	const keyPair = await crypto.subtle.generateKey(algorithm, !0, ["deriveBits"]) as CryptoKeyPair;
...
async function deriveSharedSecret(privateKey, peerPublicKey, group = "P-256") {
	...
	const deriveAlgorithm = { name: algorithm.name, public: peerKey } as AlgorithmIdentifier & { public: CryptoKey };
	return new Uint8Array(await crypto.subtle.deriveBits(deriveAlgorithm, privateKey, bits))
}
...
export class TlsClient {
	...
	cipherConfig: TlsCipherConfig | null;
	isTls13: boolean;
	...
	keyPairs: Map<number, TlsKeyShare>;
	ecdhKeyPair: CryptoKeyPair | null;
	sawCert: boolean;

	constructor(socket: SocketLike, options: TlsClientOptions = {}) {
	...
	async readHandshakeUntil(reader, predicate, closedError) {
		...
		return this.readRecordsUntil(reader, (async record => {
			if (record.type === CONTENT_TYPE_ALERT) {
				if (shouldIgnoreTlsAlert(record.fragment)) return false;
	...
	async handshakeTls12(reader, writer) {
		...
		await this.readRecordsUntil(reader, (async record => {
			...
			if (record.type === CONTENT_TYPE_CHANGE_CIPHER_SPEC) {
				sawChangeCipherSpec = !0;
				return false;
			}
			if (record.type !== CONTENT_TYPE_HANDSHAKE || !sawChangeCipherSpec) return false;
			const decrypted = await this.decryptTls12(record.fragment, CONTENT_TYPE_HANDSHAKE);
			if (decrypted[0] !== HANDSHAKE_TYPE_FINISHED) return false;
	...
	async handshakeTls13(reader, writer, serverHello) {
		...
		await this.readRecordsUntil(reader, (async record => {
			if (record.type === CONTENT_TYPE_CHANGE_CIPHER_SPEC || record.type === CONTENT_TYPE_HANDSHAKE) return false;
			if (record.type === CONTENT_TYPE_ALERT) {
				if (shouldIgnoreTlsAlert(record.fragment)) return false;
				throw new Error(`TLS Alert: ${record.fragment[1]}`);
			}
			if (record.type !== CONTENT_TYPE_APPLICATION_DATA) return false;
